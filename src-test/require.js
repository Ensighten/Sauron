...
(function () {
    ...
    // Custom sugar for MVC-based defines
    window.defineController = function (name) {
      var args = [].slice.call(arguments),
          controllerUrl = require.getContext().config.paths._controllerDir || '';
      args[0] = controllerUrl + name;
      define.apply(this, args);
    };
    window.defineModel = function (name) {
      var args = [].slice.call(arguments),
          modelUrl = require.getContext().config.paths._modelDir || '';
      args[0] = modelUrl + name;
      define.apply(this, args);
    };
    // Views have a different flavor for defining (see mvc.js)
        ...
        if (isBrowser) {
          ...
          function _xhrLoop() {
            ...
            xhr.onreadystatechange = function() {
              if (xhr.readyState === 4) {
                if (+xhr.status === 200) {
                  // TODO: Move over to new Function and better identify errors
                  try {
                    window.eval(xhr.responseText);
                    callback();
                  } catch (e) {
                    var err = new Error();
                    console && console.error && console.error('Error thrown by file ' + url + ' on line: ' + (e.lineNumber - err.lineNumber + 3), e);
                  }
                }
                else {
